 ...
 * Font sourced from lib/fonts/ (fetched by pc.py deps) or system install.
 */
public class TrueTypeFontParserTest {

    private static final Path[] DEJAVU_CANDIDATES = {
        Path.of("lib", "fonts", "DejaVuSans.ttf"),
        Path.of("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf")
    };

    private Path dejaVuSans;

    @Before
    public void findDejaVuSans() {
        for (Path candidate : DEJAVU_CANDIDATES) {
            if (Files.exists(candidate)) {
                dejaVuSans = candidate;
                return;
            }
        }
        fail("DejaVu Sans not found. Run 'python3 pc.py deps' to fetch fonts.");
    }

    @Test
    public void testDejaVuSansHeadTable() throws Exception {
        TrueTypeFontParser.clearCache();
        FontData data = TrueTypeFontParser.parse(dejaVuSans);
    ...
    public void testCaching() throws Exception {
        TrueTypeFontParser.clearCache();
        FontData first = TrueTypeFontParser.parse(dejaVuSans);
        FontData second = TrueTypeFontParser.parse(dejaVuSans);
